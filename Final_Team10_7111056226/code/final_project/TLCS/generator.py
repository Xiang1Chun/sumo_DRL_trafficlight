...
class Generator:
    def __init__(self, max_steps, car_number):
        self.car_number = car_number  # 車子數量
    ...
    def generate_routefile(self, seed):
        ...
        # 使用 Weibull 分布生成車輛生成的時間
        timings = np.random.weibull(2, self.car_number)
        timings = np.sort(timings)

        # 時間分布重新調整為適合 0 到 max_steps 的區間
        car_gen_steps = []
        ...
        with open("intersection/episode_routes.rou.xml", "w") as routes:
            ...
            <route id="W_N" edges="W1 N0"/>
            <route id="W_E" edges="W1 E0"/>
            <route id="W_S" edges="W1 S0"/>
            <route id="N_W" edges="N1 W0"/>
            <route id="N_E" edges="N1 E0"/>
            <route id="N_S" edges="N1 S0"/>
            <route id="E_W" edges="E1 W0"/>
            <route id="E_N" edges="E1 N0"/>
            <route id="E_S" edges="E1 S0"/>
            <route id="S_W" edges="S1 W0"/>
            <route id="S_N" edges="S1 N0"/>
            <route id="S_E" edges="S1 E0"/>""", file=routes)

            for car_counter, step in enumerate(car_gen_steps):
                straight_or_turn = np.random.uniform()
                if straight_or_turn < 0.70:  # 70% 直行 30% 轉彎
